 ...
 * 
 * "input x y s" ... where x and y are arbitrary int coordinates, and s is a
 * string strictly delimited by whitespace (no way to quote it or escape it).
 * This command creates a graph vertex in the graph at coordinates (x,y) and
 * associates string s with it. In this project we assume it represents a subway
 * station, and s represents the name of the station, not necessarily unique. If
 * there is already a station at coordinates (x,y) then this command updates the
 * name string. The command produces no output.
 *
 * "Adjacent x1 y1 x2 y2" . . . where x1, y1, x2, and y2 are arbitrary int
 * coordinates. If there is a station at coordinates (x1, y1) and also a station
 * at coordinates (x2, y2), then this command creates an undirected graph edge
 * between the vertices, regardless of any other geometric concerns. The edge
 * represents a subway tunnel directly connecting the stations. If either
 * station does not exist, the command has no effect. The command produces no
 * output.
 *
 * "Route x1 y1 x2 y2" . . . where x1, y1, x2, and y2 are arbitrary int
 * coordinates. If there is a station at coordinates (x1, y1) and also a station
 * at coordinates (x2, y2), then this command computes a shortest path (i.e.,
 * minimum number of stations) between stations. If the start or end station
 * does not exist, or the stations are not connected in the graph, then the
 * command prints "NO PATH"; otherwise the output consists of lines showing the
 * stations along a path in the graph.
 *
 * "Delete x y" This command removes a vertex at (x,y) and its associated edges
 * from the graph. If there is no station at coordinates (x, y), the command is
 * ignored. The command produces no output. For this assignment, this command is
 * optional (for extra credit) -- see the assignment for details.
 *
 * Example
 *
 * INPUT ===============
 * 
 * insert 1 1 red insert 1 2 orange insert 2 1 green insert 2 2 blue Adjacent 1
 * 1 1 2 Adjacent 1 1 2 1 Adjacent 1 1 2 2 Adjacent 1 2 2 2 Adjacent 2 1 2 2
 * Route 1 2 2 1
 * 
 * OUTPUT ===============
 * 
 * Station 1, 2 orange Station 1, 1 red Station 2, 1 green
 * 
 * Version 1.0
 */
public class SubwayDriver {
	...
	private static void check_for_bad_state(int cmd_state, int data_seq, boolean does_del) {
		if (RESET == cmd_state)
	...
	private static void driver(BufferedReader br, boolean does_deletion) throws FileNotFoundException, IOException {
		/*
		...
		for (String line = br.readLine(); line != null; line = br.readLine())
			for (StringTokenizer tz = new StringTokenizer(line); tz.hasMoreTokens();) {
				...
				if (RESET == cmd_state) {
					...
					/*
					 * Insert vertex into the graph (or update its satellite
					 * data). (The token contains the satellite data string.)
					 */
					 ...
					 * (start) X1IN --> Y1IN --> X2IN --> Y2IN, followed by
					 * action, RESET. We use the variables (x1, y1, x2) to store
					 * earlier coordinates, until we have collected all four.
					 *
					...
					if (X1IN == data_seq) { // read first coordinate after cmd
						...
						if (DELETE == cmd_state) {
							/*
							 * That is enough coordinates! Now delete the vertex
							 * from the graph.
							 */
						...
						if (ADJACENT == cmd_state) {
							 ...
							 */
							subway.adjacent(new Point(x1, y1), new Point(x2, coord));
						} else {
							 ...
							 * in string form.
							 */
							String route = subway.route(new Point(x1, y1), new Point(x2, coord));
